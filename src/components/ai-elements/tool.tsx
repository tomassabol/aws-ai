...
export const ToolOutput = ({
  ...
  const renderOutput = (value: unknown): ReactNode => {
    // Strings that look like JSON → pretty print
    if (typeof value === "string") {
      ...
      return (
        <pre className="p-3 whitespace-pre-wrap break-words text-xs">
          {value}
        </pre>
      );
    }

    // OpenAI-style tool output object: { content: [{ type: 'text', text }] }
    if (
      value &&
      typeof value === "object" &&
      ...
      if (texts.length > 0) {
        // Try to parse first text as JSON; otherwise render as preformatted text
        const first = texts[0]!;
        try {
    ...
    // Generic object or array → pretty JSON
  ...
  return (
    <div className={cn("space-y-2 p-4", className)} {...props}>
      ...
      >
        ...
        {typeof output !== "undefined" && <div>{renderOutput(output)}</div>}
      </div>
    </div>
  );
};
